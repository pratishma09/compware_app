...
@section('content')
    <div class="flex flex-col lg:flex-row container mx-auto py-8 pt-20">
        <div class="lg:w-4/5">
            <h1 class="text-5xl mb-4 text-blue text-center pl-6">{{ $coursess->course_name }}</h1>
            ...
            <div class="bg-white p-6 rounded shadow-md">
                <div v-for="card in cards"
                    ...
                    <div class="flex flex-col items-center justify-center w-auto md:w-1/2">
                        @if (filter_var($coursess->course_logo, FILTER_VALIDATE_URL))
                            <img class="object-cover rounded pt-3 pb-3 h-32" src="{{ $coursess->course_logo }}" />
                        @else
                            <img class="object-cover rounded pt-3 pb-3 h-32"
                                src="{{ asset('assets/' . $coursess->course_logo) }}" />
                        @endif
                    </div>
                    <div class="w-full py-4 px-6 text-gray-800 flex flex-col justify-between font-roboto">
                        <h3 class="font-medium text-2xl leading-tight">{{ $coursess->course_name }}</h3>
                        <div class="flex justify-start py-5">
                            <button type="button"
                                ...
                                data-popups-id="{{ $coursess->id }}">
                                Enroll
                            </button>
                            <button type="button"
                                class="border bg-blue hover:shadow-xl rounded px-4 py-2 ml-3 shadow-sm text-md uppercase text-white">
                                <a href="{{ asset('assets/' . $coursess->course_pdf) }}" target="_blank">View Details</a>
                            </button>
                        </div>
                    </div>
                </div>
                <p class="mb-4 pt-10">{!! $coursess->course_desc !!}</p>
            </div>
        </div>
        <div id="popups-{{ $coursess->id }}"
                ...
                <div class="w-96">
                    ...
                    <form method="post" action="{{ route('enroll.store') }}" id="enrollForm">
                        ...
                        <div class="mt-4">
                            ...
                            <input name="course_category" value="{{ $coursess->course_name }}"
        ...
        <div class="container lg:w-2/6 sm:w-1/2 pr-10 border border-gray-200 ml-20 mt-36">
            ...
            @foreach ($course as $recentcourse)
                <div class="pl-6">
                    <a href="{{ route('course.show', $recentcourse->course_slug) }}">
                        <div class="h-48 w-auto">
                            @php
                                ...
                                if (
                                    filter_var($recentcourse->course_logo, FILTER_VALIDATE_URL) &&
                                    (strpos($recentcourse->course_logo, 'http://') === 0 ||
                                        strpos($recentcourse->course_logo, 'https://') === 0)
                                ) {
                                    $imageUrl = $recentcourse->course_logo;
                                }
                            @endphp
                            <img src="{{ $imageUrl }}" alt="{{ $recentcourse->course_name }}"
                                class="mt-2 rounded-lg h-36 w-full object-cover object-center inset-0">
                        </div>
